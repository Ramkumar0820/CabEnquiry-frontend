...
const Header = () => {
  ...
  useEffect(() => {
    if (params?.slug) {
      setSearchQuery(decodeURIComponent(params.slug));
    }
  }, [params]);
  ...
  return (
    <>
      <div className="shadow-1 flex justify-between items-center px-4 py-0 sm:px-8 md:px-16 z-50 sticky top-0 bg-white">
        ...
        <div className="hidden md:flex">
          <ul className="flex gap-4 sm:gap-6">
            <Link href="/">
              <li
                className={`cursor-pointer hover:text-primary ${isActive("/")
            ...
            <Link href="/cars">
              <li
                className={`cursor-pointer hover:text-primary ${isActive("/cars")
                    ? "text-primary border-b border-primary"
                    : ""
                  }`}
              >
            ...
            <Link href="/about-us">
              <li
                className={`cursor-pointer hover:text-primary ${isActive("/about-us")
                    ? "text-primary border-b border-primary"
                    : ""
                  }`}
              >
            ...
            <Link href="/contact-us">
              <li
                className={`cursor-pointer hover:text-primary ${isActive("/contact-us")
                    ? "text-primary border-b border-primary"
                    : ""
                  }`}
              >
        ...
        <div className="relative w-48 sm:w-64 md:w-96 hidden md:flex items-center gap-4">
          <div className="relative w-full">
            ...
            <AiOutlineSearch
              className="absolute top-1/2 right-10 transform -translate-y-1/2 text-gray-500 cursor-pointer"
              size={20}
              onClick={onSearch}
            />
            ...
            {searchQuery && (
              <FaTimes
                className="absolute top-1/2 right-3 transform -translate-y-1/2 text-gray-400 cursor-pointer hover:text-red-500"
                size={16}
                onClick={() => {
                  setSearchQuery("")
                  router.push("/");
      ...
      {/* Mobile Sidebar */}
      <div
        className={`fixed inset-0 z-40 bg-black bg-opacity-50 transition-opacity ${sidebarOpen ? "opacity-100 visible" : "opacity-0 invisible"} md:hidden`}
        onClick={toggleSidebar}
      ></div>
      <div
        className={`fixed top-0 right-0 w-[90%] h-full bg-white shadow-lg z-50 transform transition-transform ${sidebarOpen ? "translate-x-0" : "translate-x-full"} md:hidden`}
      >
        ...
        <div className="relative p-4">
          ...
          <AiOutlineSearch
            className="absolute top-1/2 right-14 transform -translate-y-1/2 text-gray-500 cursor-pointer"
            size={20}
            onClick={onSearch}
          />
          ...
          {searchQuery && (
            <FaTimes
              className="absolute top-1/2 right-6 transform -translate-y-1/2 text-gray-400 cursor-pointer hover:text-red-500"
              size={16}
              onClick={() => {
                setSearchQuery("")
                router.push("/");
